$(() => {
    $("#inp_search").keyup(function () {
        if (($("#search_parameters").val() == "length" || $("#search_parameters").val() == "cost") && $("#inp_search").val() == "") {
            $.ajax({
                url: "/searchMovie?" + $("#search_parameters").val() + "=0",
            }).done(function (res) {
                $(".results").html("")
            ...
            $.ajax({
                url: "/searchMovie?" + $("#search_parameters").val() + "=" + $("#inp_search").val(),
            }).done(function (res) {
                $(".results").html("")
...
function validateMovieUpdateBeforeSubmitAfterFilter(index) {
    ...
    // Check if the trailer link is valid
        ...
        trailer_html.style.borderColor = "green"
        trailer_message.innerHTML = ""
        $(".UpdateMovieAfterFilterForm{i}" + index).submit()
    }
}